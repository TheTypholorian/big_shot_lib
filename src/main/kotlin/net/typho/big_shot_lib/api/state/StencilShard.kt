...
open class StencilShard(
    ...
    if (enabled) listOf(
        ...
        Bindable.ofState(OpenGL.INSTANCE::stencilFunc, func),
        Bindable.ofState(OpenGL.INSTANCE::stencilMask, mask),
        Bindable.ofState(OpenGL.INSTANCE::stencilOp, op),
    ) else listOf()
) {
    companion object : RenderSettingShard.Type<StencilShard> {
        ...
        override fun location(): ResourceLocation = ResourceLocation.fromNamespaceAndPath("opengl", "stencil")
    }
}