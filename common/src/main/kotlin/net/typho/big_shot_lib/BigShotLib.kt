...
object BigShotLib {
    ...
    @JvmStatic
    fun Identifier.toNeo(): ResourceIdentifier = ResourceIdentifier(namespace, path)

    @JvmStatic
    fun ResourceIdentifier.toMojang(): Identifier = Identifier.fromNamespaceAndPath(namespace, path)

    @JvmStatic
    fun <T : Any> ResourceKey<T>.toNeo(): NeoResourceKey<T> = NeoResourceKey(registry().toNeo(), identifier().toNeo())

    @JvmStatic
    fun <T : Any> NeoResourceKey<T>.toMojang(): ResourceKey<T> = ResourceKey.create(ResourceKey.createRegistryKey(registry.toMojang()), location.toMojang())

    @JvmStatic
    fun <T : Any> TagKey<T>.toNeo(): NeoTagKey<T> = NeoTagKey(registry().toNeo().location, location().toNeo())

    @JvmStatic
    fun <T : Any> NeoTagKey<T>.toMojang(): TagKey<T> = TagKey.create(ResourceKey.createRegistryKey(registry.toMojang()), location.toMojang())
